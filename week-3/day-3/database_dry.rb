...
class Menu
  ...
  def minimum_salary(search_position)
    ...
    person_with_smallest_salary = matched_people.min_by { |person| person.salary }
  ...
  def maximum_salary(search_position)
    ...
    person_with_largest_salary = matched_people.max_by { |person| person.salary }
  ...
  def employee_names(search_position)
    ...
    names = matched_people.map { |person| person.name }
  ...
  def employee_names_even_more_ruby_style(search_position)
    matching_people(search_position).
      map(&:name).
      join(",")
  end
